...
simulated once you pass `evaluating=True` to `env.reset` and your solution
passes if it gets a non-negative average return in all the tests. You can
train the agent directly in ReCodEx (the time limit is 5 minutes),
or submit a pre-trained one.

PyTorch template [memory_game.py](https://github.com/ufal/npfl139/tree/master/labs/09/memory_game.py)
